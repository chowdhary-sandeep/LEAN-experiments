...
def strip_prefix_for_repl(lines: List[str]) -> List[str]:
    ...
    while i < len(lines):
        ...
        # ── Doc comments: strip entirely (orphaned doc comments cause errors) ──
        # /-- ... -/ must be immediately before a declaration; if we strip the
        # declaration but keep the doc comment, Lean rejects it with expected_token.
        if s.startswith("/--"):
            if "-/" in s:
                i += 1  # single-line doc comment
            else:
                i += 1  # skip to closing -/
        ...
        # ── Block comment tracking (kept verbatim) ──
