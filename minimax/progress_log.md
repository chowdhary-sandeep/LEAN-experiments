# Mode 2 Verification — Progress Log

## FINAL DIAGNOSIS AND EXACT CODE (2026-03-08)

**The agent has tried three approaches, all wrong:**

| Run | Approach | Result | What went wrong |
|-----|----------|--------|-----------------|
| Run 1 | Full stripped prefix, prefix BEFORE `noncomputable section` | 2.9% | Bug 1: wrong wrapper order. Bug 2: orphaned `@[...]` lines |
| Run 2 | "Hybrid" — element extraction from prefix only | 56% | Agent reverted to Mode 1-style extraction but position-restricted — loses section-scoped variables |
| Mode 1 baseline | Element extraction from ENTIRE file | 84-90% | Correct result for Mode 1 |

**The correct approach (stripped verbatim prefix) has never been tried correctly.**

The agent keeps reverting to element-by-element extraction (extract_opens, extract_variables).
This is **WRONG**. Element extraction loses section structure. Variables inside `section S`
need to be INSIDE section S in the command, not extracted and placed globally.

The only correct implementation: **include the stripped prefix VERBATIM**, preserving all
section/namespace structure. Just remove the lines that would conflict with `import Mathlib`.

---

## EXACT REPLACEMENT CODE — Copy these functions verbatim

Replace the entire `test_corpus_mode2.py` with the code below. Do not invent variations.
...
1. **Delete the current `test_corpus_mode2.py`** and replace it with the code above verbatim.
   Do NOT modify anything. Copy exactly as written.

2. **Verify syntax:**
   ```bash
   python3 -m py_compile test_corpus_mode2.py && echo "OK"
   ```

3. **Debug check (fast, no Lean startup):**
   ```bash
   python3 -u test_corpus_mode2.py --count 3 --debug 2>&1 | head -80
   ```
   Verify the generated command looks like:
   ```
   noncomputable section        ← FIRST LINE
   set_option quotPrecheck false
   [source file lines — sections, opens, variables, no @[...], no imports, no public defs]
   private theorem foo_vt ... :=
     [proof]
   end [names]
   end
   ```
...
4. **Full run:**
   ```bash
   python3 -u test_corpus_mode2.py --count 500 --workers 4 2>&1 | tee /tmp/mode2_v3.log
   ```

5. **Expected result:** ≥90% (matching Mode 1 minimum). If section scoping helps, ≥95%.

---

## History

| Run | File version | Pass rate | Root cause of failure |
|-----|-------------|-----------|----------------------|
| Run 1 | First rewrite | 2.9% | Wrong wrapper order + orphaned @[...] |
| Run 2 | Second rewrite (hybrid) | 56% | Reverted to element extraction from prefix-only — loses section-scoped vars |
| **Run 3** | Correct stripped prefix | **7.5%** | Variable/comment at top level invalid in noncomputable section |
