...
| 10 | 5000 | 1988 | 39.8% (**REGRESSION**) | Source file variables — section-scoped, broke with duplicates + universe leaks |
| 11-12 | 1000 | 384 | 38.4% | Attempted fixes: dedup, unicode names, block comments — still regressed |
| 13 | 1000 | 504 | 50.4% | Reverted source vars. Note: 50.4% on 1000 = ~60% on first 500 + ~40% on 501-1000 |
| 14 | 1000 | 538 | 53.8% | Strip `Category.{v,u}` → `Category` in typeclass instances |
| 15 | 1000 | 546 | 54.6% | Skip typeclass instances with `✝` in type_str (R✝ references) |
| 16-20 | 1000 | 546 | 54.6% | Apostrophe names, universe decl attempts — no net gain |
| **21** | 1000 | TBD | TBD | **Merged agent2 fixes**: namespace prefix strip + local notations + universe decl (non-inst only) |
...
| Error | Count | Notes |
|-------|-------|-------|
| other | 28 | Mixed tactic failures |
| unknown_identifier | 14 | `✝`-named vars in proof body (corpus artifact, unfixable), renamed lemmas |
| type_mismatch | 11 | Application type mismatches |
| failed_to_synthesize | 11 | Typeclass synthesis failures |
| function_expected | 9 | Variable type conflicts |
| expected_token | 4 | Unicode notation, `#adaptation_note` |
| already_declared | 1 | `universe u` double-declaration |
