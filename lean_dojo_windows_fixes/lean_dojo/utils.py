...
def working_directory(
    ...
    origin = Path.cwd()
    if path is None:
        # NOTE (Windows): cleanup can fail due to transient file locks (AV/indexing, git processes).
        # Avoid crashing on cleanup errors.
        if os.name == "nt":
            tmp_dir = tempfile.TemporaryDirectory(dir=TMP_DIR, ignore_cleanup_errors=True)
        else:
...
def is_git_repo(path: Path) -> bool:
    ...
    with working_directory(path):
        # NOTE (Windows): `1>/dev/null 2>/dev/null` breaks on cmd.exe and prints
        # "The system cannot find the path specified.", causing false negatives.
        if os.name == "nt":
            cmd = "git rev-parse --is-inside-work-tree >nul 2>nul"
        else:
            cmd = "git rev-parse --is-inside-work-tree 1>/dev/null 2>/dev/null"
...
def _from_lean_path(root_dir: Path, path: Path, repo, ext: str) -> Path:
    ...
    # Defensive fix: clean any build directory fragments that may have leaked into source paths
    # This can happen if ExtractData.lean's findLean doesn't fully strip build paths
    # NOTE: Handle both forward slashes (Unix) and backslashes (Windows)
    path_str = str(path)
    # Try both separators and remove ALL occurrences (no break)
    for sep in ["/", "\\"]:
        for build_frag in [f".lake{sep}build{sep}lib{sep}lean{sep}", 
                           f".lake{sep}build{sep}lib{sep}", 
                           f"build{sep}lib{sep}lean{sep}", 
                           f"build{sep}lib{sep}"]:
            while build_frag in path_str:
                path_str = path_str.replace(build_frag, "")
    path = Path(path_str)
