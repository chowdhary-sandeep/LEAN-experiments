...
def findLean (mod : Name) : IO FilePath := do
  ...
  -- Remove "build/lib/lean/" or "build/lib/" substring from the path.
  -- NOTE: Some packages (e.g., aesop, batteries) use `.lake/build/lib/Foo.olean` (no `lean/` subdir),
  -- while lean4 stdlib uses `.lake/build/lib/lean/Init.olean`. Handle both layouts.
  -- CRITICAL: On Windows, paths use backslashes (\), so we must handle BOTH separators.
  -- Normalize to forward slashes first, then do all replacements.
  let lean := olean.toString
    |>.replace "\\" "/"  -- Normalize Windows backslashes to forward slashes
    |>.replace ".lake/build/lib/lean/" ""
    |>.replace ".lake/build/lib/" ""  -- For packages without lean/ subdir
    |>.replace "build/lib/lean/" ""
    |>.replace "build/lib/" ""  -- For packages without lean/ subdir
  ...
  -- NOTE: Some toolchains / packages don't have source `.lean` available at the inferred path.
  -- This path is only used for dependency listing (`*.dep_paths`); don't crash tracing if missing.
...
-- NOTE (compatibility): In some Lean versions `Parser.parseHeader` returns `Syntax`, not `TSyntax`.
...
def shouldProcess (path : FilePath) (noDeps : Bool) : IO Bool := do
  ...
  -- NOTE (compatibility): some toolchains place `.olean` under `.lake/build/lib/<File>.olean`
  -- instead of `.lake/build/lib/lean/<File>.olean`. Accept either so project files (e.g. Lean4Example.lean)
  -- are actually traced and produce `.ast.json/.dep_paths`, enabling `.trace.xml` generation.
  let some oleanPath1 := Path.toBuildDir "lib/lean" relativePath "olean" |
    throw $ IO.userError s!"Invalid path: {path}"
  let some oleanPath2 := Path.toBuildDir "lib" relativePath "olean" |
    throw $ IO.userError s!"Invalid path: {path}"
  return (← oleanPath1.pathExists) ∨ (← oleanPath2.pathExists)
