...
def _split_git_url(url: str) -> Tuple[str, str]:
    ...
    # NOTE (Windows/local-path compatibility):
    # - For local repos, LeanGitRepo.from_path() passes a filesystem path as `url`.
    # - On Windows this path uses backslashes, so splitting only on "/" can yield < 2 fields.
    fields = [f for f in re.split(r"[\\/]+", url) if f]
    if len(fields) >= 2:
        user_name = fields[-2]
        repo_name = fields[-1]
    elif len(fields) == 1:
...
class LeanFile:
    ...
    def __post_init__(self) -> None:
        ...
        for line in self.abs_path.open("rb"):
            # Normalize Windows-style line endings (CRLF -> LF) for cross-platform compatibility
            if line.endswith(b"\r\n"):
                endwith_newline = True
                line = line[:-2]  # Strip both \r and \n
            ...
            num_bytes.append(len(line) + 1)  # +1 for the normalized \n
...
class LeanGitRepo:
    ...
    def get_dependencies(
        ...
        toolchain = (
            ...
            if path is None
            else {"content": (Path(path) / "lean-toolchain").open(encoding="utf-8").read()}
        )
        ...
        try:
            lake_manifest = (
                ...
                if path is None
                else json.load((Path(path) / "lake-manifest.json").open(encoding="utf-8"))
            )
    ...
    def _parse_lakefile_lean_dependencies(
        ...
        lakefile = (
            ...
            if path is None
            else (Path(path) / "lakefile.lean").open(encoding="utf-8").read()
        )
    ...
    def _parse_lakefile_toml_dependencies(
        ...
        lakefile = (
            ...
            if path is None
            else (Path(path) / "lakefile.toml").open(encoding="utf-8").read()
        )
        # Parsing worked
    ...
    def get_license(self) -> Optional[str]:
        ...
        if self.repo_type == RepoType.GITHUB:
            ...
            if license_path.exists():
                return license_path.open("r", encoding="utf-8").read()
            else:
                return None
