...
class TracedFile:
    ...
    def _from_lean4_traced_file(
        ...
        # NOTE (Windows): default text encoding may be cp1252, but JSON is written as UTF-8.
        data = json.load(json_path.open(encoding="utf-8"))

        data["module_paths"] = []
        deps_path = json_path.with_suffix("").with_suffix("").with_suffix(".dep_paths")

        # NOTE: Some repos / toolchains may fail to generate `.dep_paths` for a subset of files.
        # Treat missing deps as "unknown imports" instead of crashing the whole trace load.
        if deps_path.exists():
            for line in deps_path.open(encoding="utf-8"):
                line = line.strip()
                if line == "":
                    break
...
def _save_xml_to_disk(tf: TracedFile) -> None:
    ...
    # NOTE (Windows): default text encoding may be cp1252; write XML as UTF-8.
    with xml_path.open("wt", encoding="utf-8") as oup:
        oup.write(tf.to_xml())
...
class TracedRepo:
    ...
    def check_sanity(self) -> None:
        ...
        if self.traced_files_graph is not None:
            ...
            for path_str, tf_node in self.traced_files_graph.nodes.items():
                ...
                dep_rel = to_dep_path(self.root_dir, path, self.repo)
                if dep_rel not in path_files:
                    # NOTE: Some toolchains/repos may fail to generate `.dep_paths` for a subset of files.
                    # Don't hard-fail sanity checks on Windows; keep going with missing import metadata.
                    logger.warning(f"Missing {dep_rel}")
                json_rel = to_json_path(self.root_dir, path, self.repo)
                if json_rel not in json_files:
                    logger.warning(f"Missing {json_rel}")
                if len(xml_files) > 0:
                    xml_rel = to_xml_path(self.root_dir, path, self.repo)
                    if xml_rel not in xml_files:
                        logger.warning(f"Missing {xml_rel}")
    ...
    def get_traced_file(self, path: Union[str, Path]) -> TracedFile:
        ...
        # On Windows, graph nodes use backslashes; user may pass forward slashes
        path_str = str(path)
        # Try the path as-is first
        if path_str in self.traced_files_graph.nodes:
            return self.traced_files_graph.nodes[path_str]["traced_file"]
        # Try with normalized separators (convert / to \ on Windows, \ to / on Unix)
        import os
        if os.sep == "\\":
            alt_path = path_str.replace("/", "\\")
        else:
            alt_path = path_str.replace("\\", "/")
        if alt_path in self.traced_files_graph.nodes:
            return self.traced_files_graph.nodes[alt_path]["traced_file"]
        # If still not found, raise KeyError with helpful message
        raise KeyError(f"Traced file not found: {path_str} (also tried: {alt_path})")
