...
# Windows compatibility: use PopenSpawn instead of spawn
if os.name == "nt":
    from pexpect.popen_spawn import PopenSpawn
    # PopenSpawn doesn't have isalive(), add it as a method
    def _isalive(self):
        """Check if the process is still alive (Windows-compatible)."""
        return self.proc.poll() is None
    PopenSpawn.isalive = _isalive
...
def check_proof(thm: Theorem, proof: str) -> bool:
    ...
    # NOTE: Explicitly use UTF-8 encoding to handle Unicode characters (Greek letters, etc.)
...
class Dojo:
    ...
    def __enter__(self) -> Tuple["Dojo", State]:
        ...
        with working_directory(traced_repo_path):
            ...
            # Windows compatibility: use PopenSpawn instead of spawn
            if os.name == "nt":
                # PopenSpawn on Windows doesn't split command strings automatically,
                # so we need to split it ourselves. Use shlex.split but preserve Windows paths.
                cmd_list = shlex.split(cmd, posix=False)
                self.proc = PopenSpawn(
                    cmd_list, timeout=self.timeout, maxread=1, encoding="utf-8", cwd=str(traced_repo_path)
                )
            else:
        ...
        try:
            ...
            # On Windows, capture error output if process exited
            if os.name == "nt" and isinstance(self.proc, PopenSpawn):
                if not self.proc.isalive():
                    # Process exited, try to read any remaining output
                    try:
                        import select
                        import sys as sys_module
                        # Read any remaining output
                        remaining = ""
                        if hasattr(self.proc, '_read_queue'):
                            while True:
                                try:
                                    data = self.proc._read_queue.get_nowait()
                                    if data is None:
                                        break
                                    remaining += self.proc._decoder.decode(data, final=True)
                                except:
                                    break
                        if remaining:
                            logger.error(f"Process output before exit: {remaining}")
                    except:
                        pass
                    # Check exit code
                    exit_code = self.proc.proc.poll()
            ...
            if traced_file.has_prelude:
                ...
                if os.name == "nt" and isinstance(self.proc, PopenSpawn):
                    exit_code = self.proc.proc.poll() if hasattr(self.proc, 'proc') else None
                    if exit_code is not None:
                        error_msg += f" (process exited with code {exit_code})"
                raise DojoInitError(error_msg)
    ...
    def __exit__(self, exc_type: None, exc_val: None, exc_tb: None) -> None:
        ...
        # Windows compatibility: manually delete the temp file since we used delete=False
        modified_file_path = self.modified_file.name
    ...
    def _modify_file(self, traced_file: TracedFile) -> None:
        # Windows compatibility: delete=True keeps the file open and prevents
        # other processes from reading it. Use delete=False on Windows and
        # manage deletion manually in __exit__.
        # NOTE: Explicitly use UTF-8 encoding to handle Unicode characters (Greek letters, etc.)
        self.modified_file = tempfile.NamedTemporaryFile(  # type: ignore
            ...
            dir=traced_file.abs_path.parent,
            delete=(os.name != "nt"),  # Don't auto-delete on Windows
        ).__enter__()
    ...
    def _check_alive(self) -> None:
        if self.proc.isalive():
            return
        # Windows compatibility: PopenSpawn might need poll() to set exitstatus
        if os.name == "nt" and hasattr(self.proc, 'proc'):
            # PopenSpawn wraps subprocess.Popen in self.proc
            exit_code = self.proc.proc.poll()
            if exit_code is not None:
                self.proc.exitstatus = exit_code
    ...
    def _read_next_line(self) -> Tuple[str, str]:
        ...
        _REPL_PROMPT = "REPL>"
        msg: List[str] = []
        # Windows compatibility: handle both \n and \r\n line endings
        # Use \r?\n to match optional \r before \n
        newline_pattern = r"\r?\n"
        repl_pattern = f"{_REPL_PROMPT}.*?{newline_pattern}"
        while True:
            try:
                index = self.proc.expect([newline_pattern, repl_pattern])
                if index == 0:
                    before = self.proc.before if self.proc.before else ""
                    if before == "":
                        raise EOFError
                    else:
                        ...
                        continue
                self._check_alive()
                # Extract the JSON after REPL> prompt
                match_str = self.proc.match.string if self.proc.match else ""
                res = match_str[len(_REPL_PROMPT):].strip()
                before = self.proc.before if self.proc.before else ""
                return res, "\n".join(msg) + before
            except pexpect.EOF:
                raise EOFError
