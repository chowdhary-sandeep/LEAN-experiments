...
def _extract_variables(entry: dict) -> list[str]:
    ...
    variables: list[str] = []
    for raw_line in state_before.split("\n"):
        # Skip indented continuation lines of multi-line hypotheses.
        # In Lean's state_before format, continuation lines (e.g. of h₂ : ∀ ...) are indented.
        ...
        if any(n.lower().startswith("inst") for n in names):
            ...
            if "✝" in type_str:
                continue
            # Skip ∀-quantified inst types — 'variable [∀ ...]' is invalid Lean syntax.
            # e.g. inst✝ : ∀ (i : 𝒰.J), HasPullback (𝒰.map i ≫ f) g
            if type_str.startswith("∀"):
                continue
            # Skip inst types that reference explicit_params variables.
            # When type_str references e.g. `α` or `ι` which are explicit params of the theorem,
            # those vars aren't in scope at variable-declaration time. Lean would auto-bind them
            # with unknown types (?m.XX), and then the theorem's explicit binding conflicts,
            # producing 'function expected at α term has type ?m.XX' errors.
            # Since the theorem explicitly declares these typeclass constraints (e.g. [∀ i, C (α i)]),
            # we can safely skip them here.
        ...
        if all("✝" in n for n in names):
            continue
        # Skip lines where the TYPE contains inaccessible variable references (✝).
        # e.g. b : ℍ[R,c₁✝,c₂✝] — c₁✝ and c₂✝ are inaccessible, can't use in variable decl.
        if "✝" in type_str:
            continue

        # Skip ∀-quantified hypotheses — these are always Prop-valued propositions,
        # not data types (e.g. hf : ∀ (i j : ι) (hij : i ≤ j), ...).
        if type_str.startswith("∀"):
            continue
        ...
        # Also handle function-to-type: G : ι → Type w, A : ι → Type u_3
...
def build_check_command(entry: dict) -> str:
    ...
    # Strip #adaptation_note lines — Lean 4 requires them to be followed by a doc comment,
    # but the corpus sometimes omits the required /-- ... -/ comment, causing parse failures.
    ...
    # Disable quotation precheck to avoid "unknown identifier at quotation precheck"
    # errors from dot-notation in term-mode quotations (e.g. D.toGlueData).
    lines.append("set_option quotPrecheck false")
    if flat_univs:
        lines.append(f"universe {' '.join(sorted(flat_univs))}")
    lines += local_notations
    if ns:
        # Move opens and var_decls INSIDE the namespace block so that short
        # namespace names like "Spec", "Category", "NatIso" resolve correctly
        # via Lean 4's namespace-relative open resolution.
        # E.g. inside `namespace AlgebraicGeometry`, `open Spec` resolves to
        # `AlgebraicGeometry.Spec` — but at top level it fails as unknown.
