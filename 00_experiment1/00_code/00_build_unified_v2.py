...
_JSONS_DIR = _PROJECT_DIR / "jsons"
OUTPUT_FILE = str(_JSONS_DIR / "traced_theorems_unified_v2.jsonl")
CORPUS_FILE = str(_JSONS_DIR / "corpus.jsonl")
STATS_FILE = str(_JSONS_DIR / "theorem_stats_v2.json")
PREMISE_INDEX_FILE = str(_JSONS_DIR / "premise_index_v2.json")
...
# =============================================================================
# LEGACY FIX: filter tactic/hyp premises from an existing JSONL
# Run with:  python 00_build_unified_v2.py --fix
# Only needed for JSONL files built before the TACTIC_OR_HYP_FILTER was added.
# New builds (via build_unified_v2) already filter at construction time.
# =============================================================================

def _filter_premises(premises_list):
    """Return list of premises with tactic/hyp entries removed."""
    return [p for p in premises_list
            if not _is_tactic_or_hyp(p.get("full_name") or p.get("surface_name", ""))]


def _fix_theorem_record(entry):
    """Filter premises and all_premises, recompute metrics and quality. Mutates entry."""
    from collections import defaultdict as _dd
    tactics_list = entry.get("tactics") or []
    all_premises = _dd(lambda: {"count": 0, "tactics": [], "confidence_sum": 0.0})

    for idx, tac in enumerate(tactics_list):
        filtered = _filter_premises(tac.get("premises") or [])
    ...
    metrics["avg_premise_confidence"] = (
        sum(v["confidence_sum"] / max(1, v["count"]) for v in all_premises.values())
        / max(1, len(all_premises)) if all_premises else 0
    )
    ...
    quality["all_premises_resolved"] = all(
        ...
        for p in (t.get("premises") or [])
    )
    entry["quality"] = quality
    return entry


def _needs_fix(data_file, sample=200):
    """Return True if JSONL contains unfiltered tactic/hyp premises (sample first N tactic records)."""
    n = 0
    try:
        with open(data_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if entry.get("proof_type") != "tactic":
                    continue
                for fn in (entry.get("all_premises") or {}):
...
def apply_fix_if_needed(data_file):
    """Filter tactic/hyp premises from an existing JSONL in-place, only if needed.

    Safe to call unconditionally — does nothing if the file is already clean.
    Replaces file atomically via a temp file in the same directory.
    """
    import os, sys, tempfile
    data_file = Path(data_file)
    if not data_file.exists():
        print(f"[fix] File not found: {data_file}", file=sys.stderr)
        return False

    if not _needs_fix(data_file):
        print(f"[fix] {data_file.name} is already clean — no fix needed.")
        return False

    print(f"[fix] Filtering tactic/hyp premises from {data_file.name} ...")
    n = 0
    with open(data_file, "r", encoding="utf-8") as f_in,          tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", suffix=".jsonl",
                                     delete=False, dir=data_file.parent) as f_out:
        tmp_path = f_out.name
        for line in f_in:
            line = line.strip()
            if not line:
            ...
            try:
                ...
                continue
            _fix_theorem_record(entry)
            f_out.write(json.dumps(entry, ensure_ascii=False) + "
")
            n += 1

    os.replace(tmp_path, data_file)
    print(f"[fix] Done — rewrote {n:,} records to {data_file.name}")
    return True
...
if __name__ == "__main__":
    import sys as _sys
    if "--fix" in _sys.argv:
        apply_fix_if_needed(Path(OUTPUT_FILE))
    else:
        ...
        print()
        print("To fix an existing JSONL built before tactic filtering was added:")
        print("  python 00_code/00_build_unified_v2.py --fix")
