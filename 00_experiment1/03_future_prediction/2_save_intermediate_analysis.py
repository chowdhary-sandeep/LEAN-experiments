...
def load_pipeline_module():
    script = ROOT / "2_future_prediction_pipeline_v2.py"
...
def load_cached_objects():
    with open(DATA_DIR / "2_fvs_pipeline_v2_dag.pkl", "rb") as f:
        dag = pickle.load(f)
    with open(DATA_DIR / "2_fvs_pipeline_v2_depths.pkl", "rb") as f:
        depths = pickle.load(f)
    with open(DATA_DIR / "2_fvs_pipeline_v2_targets.pkl", "rb") as f:
...
def save_figures(target_df, split_summary_df, importance_summary_df):
    ...
    fig.tight_layout()
    fig.savefig(FIGS_DIR / "2_intermediate_target_and_stability.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(FIGS_DIR / "2_intermediate_y2_feature_importance_r0.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def save_summary_markdown(target_df, split_summary_df, importance_summary_df):
    ...
    (DATA_DIR / "2_intermediate_analysis_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def main():
    ...
    feature_cfg = pipeline.load_feature_config(ROOT / "2_future_prediction_pipeline_v2.json")
    ...
    target_df.to_csv(DATA_DIR / "2_intermediate_target_structure.csv", index=False)
    bucket_df.to_csv(DATA_DIR / "2_intermediate_y1_by_y2_bucket.csv", index=False)
    feature_corr_df.to_csv(DATA_DIR / "2_intermediate_feature_correlations_r0.csv", index=False)
    feature_corr_summary_df.to_csv(DATA_DIR / "2_intermediate_feature_correlations_r0_summary.csv", index=False)
    split_df.to_csv(DATA_DIR / "2_intermediate_repeated_split_metrics_r0.csv", index=False)
    split_summary_df.to_csv(DATA_DIR / "2_intermediate_repeated_split_summary_r0.csv", index=False)
    importance_df.to_csv(DATA_DIR / "2_intermediate_y2_feature_importance_r0.csv", index=False)
    importance_summary_df.to_csv(DATA_DIR / "2_intermediate_y2_feature_importance_r0_summary.csv", index=False)
    ...
    manifest = {
        "data_files": sorted(path.name for path in DATA_DIR.glob("2_intermediate_*")),
        "figure_files": sorted(path.name for path in FIGS_DIR.glob("2_intermediate_*")),
    }
    (DATA_DIR / "2_intermediate_analysis_manifest.json").write_text(
