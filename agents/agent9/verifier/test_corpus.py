...
def _extract_variables(entry: dict) -> list[str]:
    ...
    # Parse ALL names in multi-name binders: '{X Y : Scheme}', '(i j : ι)', etc.
    # Only extract from the PARAMETER section (before the ':' that starts the return type),
    # not from the return type or proof body where binder-like syntax can appear.
    explicit_params: set[str] = set()
    # Strip @[attr] blocks — they may contain ':' and pollute param extraction.
    _stmt_clean = re.sub(r"@\[.*?\]", "", statement, flags=re.DOTALL)
    # Skip past any leading modifiers and the theorem/lemma/def/abbrev keyword + name.
    _sig_m = re.search(r"\b(?:theorem|lemma|def|abbrev)\s+\S+", _stmt_clean)
    if _sig_m:
        _rest = _stmt_clean[_sig_m.end():]
        # Find the cut point: first ':' at bracket depth 0 that isn't ':='
        _depth = 0
        _cut = len(_rest)
        for _j, _ch in enumerate(_rest):
            if _ch in "({[⦃":
                _depth += 1
            elif _ch in ")}]⦄":
                _depth -= 1
            elif _ch == ":" and _depth == 0:
                if _j + 1 < len(_rest) and _rest[_j + 1] == "=":
                    continue  # ':=' is assignment, not type separator
                _cut = _j
                break
        _param_str = _rest[:_cut]
        for _m in re.finditer(r"[({⦃\[](.*?)[)}\]⦄]", _param_str, re.DOTALL):
            _content = _m.group(1).strip()
            if ":" in _content:
                _names_part = _content.split(":", 1)[0].strip()
                for _name in _names_part.split():
                    if re.match(r"^[^\W\d]\w*'*$", _name, re.UNICODE):
                        explicit_params.add(_name)

    # First pass: collect which names are declared as Type/Sort variables in state_before.
    # This is needed to detect inst lines that reference explicit_params-only variables
    # (which are not declared as section variables, causing unbound identifier errors).
    declared_type_vars: set[str] = set()
    for line in state_before.split("\n"):
        ...
        type_str_ = line[sep + 3:].strip()
        names_ = names_str.split()
        if any(n.lower().startswith("inst") for n in names_):
            continue
        if all("✝" in n for n in names_):
            continue
        if re.search(r"\bType\b|\bSort\b", type_str_):
            for name in names_:
                if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                    declared_type_vars.add(name)
    ...
    for line in state_before.split("\n"):
        ...
        if any(n.lower().startswith("inst") for n in names):
            ...
            if "✝" in type_str:
                continue
            # Skip ∀-quantified inst types — these are invalid as 'variable [∀ ...]'
            # e.g. inst✝ : ∀ (i : 𝒰.J), HasPullback (𝒰.map i ≫ f) g
            if type_str.startswith("∀"):
                continue
            # Skip inst lines that reference explicit_params vars that aren't declared
            # as section variables. e.g. inst✝ : NontriviallyNormedField 𝕜 where 𝕜 is
            # in explicit_params but not in declared_type_vars → 𝕜 is unbound at section level.
            # Collect only FREE (non-bound) identifiers in type_str — bound names introduced by
            # binder notation '(x : T) → ...' or '∀ x, ...' in the type are NOT free vars.
            # Remove bound variables introduced within the type expression itself.
            _type_free = re.sub(r"\((.*?)\)\s*→", "", type_str)  # remove (binder : T) →
            _type_free = re.sub(r"∀\s*[^,]*,", "", _type_free)   # remove ∀ binders
            type_ids = set(re.findall(r"[^\W\d]\w*", _type_free, re.UNICODE))
            if type_ids & (explicit_params - declared_type_vars):
                continue
        ...
        # Skip lines where type_str contains inaccessible names (✝) — these reference
        # shadow variables that don't exist in our context.
        # e.g. 'a✝ b c : ℍ[R,c₁✝,c₂✝]' — b and c would get type ℍ[R,c₁✝,c₂✝] which is invalid
        if "✝" in type_str:
            continue
        ...
        if any(c in type_str for c in ["↔", "∧", "∨", "¬"]):
            continue
        # Skip ∀-quantified hypotheses (always Prop-valued, never data types)
        # e.g. hf : ∀ (i j : ι) (hij : i ≤ j), ..., Hg : ∀ (i j : ι) ..., w : ∀ ⦃x y⦄, ...
        if type_str.startswith("∀"):
            continue
        if "→" in type_str and all(
            ...
            continue
        # Skip h* and 'this' named variables that are Prop-valued (not Type/Sort)
        # e.g. hf : BddAbove (range f), ha : a ≠ 0, hX : IrreducibleSpace X
        if all(re.match(r"^h[a-zA-Z0-9_]*$", n) or n in ("this",) for n in names):
            if not re.search(r"\bType\b|\bSort\b", type_str):
                continue

        # Type universe variables (R : Type u_1, α : Type*, etc.)
        # Also handle function-type-to-Type: G : ι → Type w, A : ι → Type u_3
        if re.search(r"\bType\b|\bSort\b", type_str):
            # Normalize all Type/Sort universe level annotations
            ...
            if "→" not in type_str:
                # Pure type variable: emit as implicit {Name : Type*}
                for name in names:
                    if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                        variables.append(f"variable {{{name} : {normalized}}}")
            else:
                # Function-to-type variable: emit as implicit {Name : ι → Type*}
                for name in names:
                    if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                        variables.append(f"variable {{{name} : {normalized}}}")
            continue
