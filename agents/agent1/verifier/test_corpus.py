...
def _open_stmts(entry: dict) -> list[str]:
    ...
    # 4. Namespace-implied opens: if the theorem lives in a sub-namespace of a parent,
    # the parent namespace's definitions are in scope in the original file but not in
    # our generated code. Add the top-level open for known problematic namespaces.
    full_name = entry.get("full_name", "")
    open_ns_list = entry.get("open_namespaces") or []
    # AlgebraicGeometry: if a theorem is in the AlgebraicGeometry module, 'Scheme',
    # 'Opens' etc. are in scope because the file is inside 'namespace AlgebraicGeometry'.
    if (full_name.startswith("AlgebraicGeometry")
            or any("AlgebraicGeometry" in ns for ns in open_ns_list if not ns.startswith("Mathlib"))):
...
def _normalize_type_universes(type_str: str) -> str:
    """
    Normalize universe-level variables in a type string:
      - Named universe params in braces: Category.{v, u} → Category
      - Single/multi-letter lowercase universe args: Type u, Type w, Sort v → Type*, Sort*
      - Indexed: Type u_1, Sort u_2 → Type*, Sort*
      - Expression forms: Type (max u v) → Type*
    """
    # Strip explicit universe params from qualified names: Category.{v, u} → Category
    result = re.sub(r'(\b[A-Z]\w*)\.\{[^}]{1,60}\}', r'\1', type_str)
    # Normalize Type/Sort with any universe expression
    # Covers: Type u, Type w, Type u_1, Type (max u v), Type (u + 1)
...
def _type_has_inaccessible_args(type_str: str) -> bool:
    """
    Return True if type_str contains ✝-suffixed identifiers that are NOT 'inst✝...'
    such as 'R✝', 'M✝', 'f✝'. These are inaccessible binders that are not valid
    Lean identifiers when used as arguments in variable declarations.
    """
    # Match word chars followed by ✝ — but exclude inst✝
...
def _extract_variables(entry: dict) -> list[str]:
    ...
    # Handle multi-name binders: (f g : Type) → both f and g are explicit params.
    explicit_params: set[str] = set()
    for binder_m in re.finditer(r'[({⦃\[]\s*([^:(){}[\]]+)\s*:', statement):
        names_chunk = binder_m.group(1).strip()
        for tok in names_chunk.split():
            tok = tok.strip()
            if re.match(r'^[^\W\d]\w*$', tok, re.UNICODE):
                explicit_params.add(tok)
    ...
    for line in state_before.split("\n"):
        ...
        if any(n.lower().startswith("inst") for n in names):
            # Skip inst lines whose type contains inaccessible (✝-suffixed) arguments.
            # e.g. "inst✝ : Semiring R✝" — R✝ is not a valid Lean identifier.
            if _type_has_inaccessible_args(type_str):
                continue
            # Normalize universe params in the typeclass type
            # e.g. Category.{v, u} C → Category C
        ...
        # Also skip lines whose type references inaccessible binders (✝ args)
        # e.g. "Hg : ∀ ... (g✝ j) ..." is a hypothesis using inaccessible context
        if _type_has_inaccessible_args(type_str):
            continue

        # Type universe variables (R : Type u_1, α : Type*, etc.)
        # Also handles function types like G : ι → Type w (normalize the Type w part)
        if re.search(r"\bType\b|\bSort\b", type_str):
            normalized = _normalize_type_universes(type_str)
            for name in names:
                if re.match(r"^[^\W\d]\w*$", name) and name not in explicit_params:
                    # Use implicit braces for plain Type/Sort carrier vars (no → in type)
                    if "→" not in type_str:
...
def build_check_command(entry: dict) -> str:
    ...
    # Detect named universe variables in the statement (e.g. {R : Type uR}).
    # These are named universe levels (not u_1 patterns) that need `universe` declarations.
    # Pattern: Type/Sort followed by a lowercase name that isn't just a single digit suffix.
    ...
    # Exclude indexed universes (u_1, u_2, ...) and standalone u/v/w which are also valid
    # named universe args that need declaration
    _named_univ = {u for u in _named_univ if not re.match(r'^u_\d+$', u)}

    # Detect if statement or proof uses the ~ᵤ Associated notation (local in Mathlib).
    # If so, we re-declare it locally so it's in scope.
