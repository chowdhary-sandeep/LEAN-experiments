...
def _open_stmts(entry: dict) -> list[str]:
    ...
    # 4. Always open the theorem's own namespace components so that short typeclass
    # names (defined inside that namespace) are accessible to 'variable' declarations.
    # E.g. GradedMonoid.mk_list_dProd needs 'open GradedMonoid' for 'variable [GMonoid A]'.
    full_name = entry.get("full_name", "")
    if full_name and "." in full_name:
        ns = full_name.rsplit(".", 1)[0]  # e.g. "GradedMonoid" from "GradedMonoid.mk_list_dProd"
        # Open each namespace component (skip Lean meta namespaces)
        parts = ns.split(".")
        for part in parts:
...
def _extract_variables(entry: dict) -> list[str]:
    ...
    # Handle multi-name binders like (i j k : T) by extracting all names before ':'.
    # Pattern: opening bracket, then one or more identifiers, then ':'
    ...
    for line in state_before.split("\n"):
        ...
        if any(n.lower().startswith("inst") for n in names):
            ...
            # Skip invalid binder annotations — types that are NOT valid typeclass instances:
            # Only skip dot-projection methods on variables: μ.IsAddLeftInvariant, S.Quasicategory, X.HasMap
            # These are Prop-valued field projections on variable names, not typeclass names.
            # Note: Pi-types (i : ι) → T and forall ∀ i, T ARE valid in Lean 4 instance binders.
            tc_root = tc_type.split()[0] if tc_type.split() else ""
            # Dot-projection methods on variables: μ.IsAddLeftInvariant, S.Quasicategory, X.HasMap
            # Exclude genuine namespace paths like AlgebraicGeometry.IsOpenImmersion (multi-char prefix)
            if "." in tc_root:
                prefix = tc_root.split(".")[0]
                # Skip if prefix is a lowercase name, a single uppercase letter, or a short var (≤2 chars)
                if len(prefix) <= 2 or prefix[0].islower():
                    continue
                # Also skip if the whole root looks like Var.Method (short prefix followed by PascalCase method)
                if re.match(r'^[A-Z][a-z]?\.[A-Z]', tc_root):
                    continue
            # Forall types: ∀ (i : 𝒰.J), HasPullback (𝒰.map i ≫ f) g
            # When the bound variable has a dot-projection type (𝒰.J), strip the type annotation
            # to get ∀ i, HasPullback ... — matching how Mathlib source files write these.
            # The dot-projected type (𝒰.J) is not in scope as a standalone type name in variable decls.
            if tc_root.startswith("∀") or tc_root == "forall":
                if re.search(r'\(\w+\s*:\s*[\w𝒰]+\.\w+\)', tc_type):
                    # Strip "(name : Var.Field)" → just "name"
        ...
        # Normalize auto-generated universe levels (u_1, u_2, ...) to Type* in type strings.
        # These appear in function types like "ι → Type u_3" where u_3 is not in scope.
...
def build_check_command(entry: dict) -> str:
    ...
    # Suppress quotation precheck so dot-notation identifiers like D.toGlueData work
    # inside quoted terms (conv, show, pattern-matching in simp). Without this,
    # Lean rejects `D.toGlueData` in quote precheck because D is a variable.
    lines.append("set_option quotPrecheck false")
    if flat_univs:
