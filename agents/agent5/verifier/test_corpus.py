...
def _get_source_opens(entry: dict) -> list[str]:
    ...
    Namespace-aware: tracks the active namespace stack so that opens issued
    inside `namespace NS` are (potentially) qualified.

    When `open X` is inside `namespace A.B`, we check if `X` is a known
    sub-namespace of A.B in that same file (i.e., if `namespace X` appears
    inside `namespace A.B` in the file). If so, we qualify to `open A.B.X`.
    Otherwise we keep `open X` as-is (it's likely a top-level namespace).
    This prevents "unknown namespace 'X'" for relative sub-namespace opens
    while preserving top-level opens like `open CategoryTheory`.
    """
    if not MATHLIB_ROOT:
        return []
    ...
    # First pass: collect all sub-namespace declarations seen at each context.
    # For each namespace context (tuple of ns_stack), record which namespace names
    # are opened within it. This lets us identify: if `namespace X` appears inside
    # `namespace A.B`, then `open X` there should be qualified as `open A.B.X`.
    # We build: set of (parent_ns_path, sub_ns_name) pairs, where parent_ns_path is
    # the "."-joined ns_stack at the time the sub-namespace is declared.
    # We ALSO collect "dot-notation virtual namespaces": if `def X.foo` or
    # `theorem X.foo` appears inside `namespace A.B`, then `X` is a virtual
    # sub-namespace of A.B and `open X` inside A.B should become `open A.B.X`.
    sub_namespaces: set[tuple[str, str]] = set()
    ns_stack_p: list[str] = []
    in_block_p = False
    for line in source.splitlines():
        ...
        if not in_block_p:
            if s.startswith("/-"):
                if not (s.count("/-") == s.count("-/") and "-/" in s):
                    in_block_p = True
                continue
        else:
            if "-/" in s:
        ...
        if s.startswith("--"):
            continue
        ns_m = re.match(r'^namespace\s+(\S+)', s)
        if ns_m:
            ns_name = ns_m.group(1)
            if ns_stack_p:
                # Record: sub_namespace ns_name is declared inside ns_stack_p
                sub_namespaces.add((".".join(ns_stack_p), ns_name))
            ns_stack_p.append(ns_name)
        elif re.match(r'^end\s+(\S+)', s):
            end_name = s.split()[1]
            if ns_stack_p and ns_stack_p[-1] == end_name:
                ns_stack_p.pop()
        elif ns_stack_p:
            # Detect dot-notation virtual namespaces: `def X.foo`, `theorem X.foo`, etc.
            # These create `X` as a virtual sub-namespace of the current ns_stack
            dot_m = re.match(
                r'^(?:private\s+|protected\s+|@\[.*?\]\s*)?'
                r'(?:noncomputable\s+)?(?:def|theorem|lemma|abbrev|instance)\s+'
                r'([A-Z][A-Za-z0-9_]*)\.', s
            )
            if dot_m:
                virt_name = dot_m.group(1)
                parent_path = ".".join(ns_stack_p)
                sub_namespaces.add((parent_path, virt_name))

    seen: set[str] = set()
    in_block_comment = False
    # Track active namespace stack for qualification of relative opens
    ns_stack: list[str] = []
    ...
    for line in source.splitlines():
        ...
        if stripped.startswith("--"):
            continue
        # Track namespace open/close for qualifying relative opens
        ns_m = re.match(r'^namespace\s+(\S+)', stripped)
        if ns_m:
            ns_stack.append(ns_m.group(1))
            continue
        end_m = re.match(r'^end\s+(\S+)', stripped)
        if end_m:
            end_name = end_m.group(1)
        ...
        if m:
            ...
            for name in names_part.split():
                ...
                if name in _META_OPENS_SKIP:
                    continue
                # Qualify relative opens: if inside namespace NS1.NS2 and `name`
                # is declared as a sub-namespace of NS1.NS2 in this file, emit
                # `open NS1.NS2.name`. Otherwise keep `open name` as-is.
                # Note: if name already has a dot (e.g., CategoryTheory.Limits), keep.
                if ns_stack and "." not in name:
                    parent_path = ".".join(ns_stack)
                    if (parent_path, name) in sub_namespaces:
                        # It's a known sub-namespace — qualify it
                        qualified_name = parent_path + "." + name
                        key = f"open{scoped} {qualified_name}"
                    else:
                        # Likely a top-level namespace — keep unqualified
...
# Cache of source-file variable lines: (file_path, ns_key) -> [variable statements]
# ns_key is the namespace context to filter variables by (None = all)
_source_vars_cache: dict[tuple, list[str]] = {}


def _get_source_variables(entry: dict) -> list[str]:
    """
    Read the actual .lean source file and extract variable declarations that are
    in scope for the theorem's namespace. Returns list of raw 'variable ...' strings.
    Cached per (file_path, theorem_namespace).
    ...
    Namespace filtering: we only include variables declared at the file top-level
    (outside all namespaces) OR inside the same namespace chain as the theorem.
    Variables declared inside a DIFFERENT namespace (e.g., inside `namespace Finset`
    when the theorem is in `namespace ENNReal`) are excluded to avoid wrong types.
    """
    if not MATHLIB_ROOT:
        return []
    file_field = entry.get("file", "")
    ...
    # Determine the theorem's namespace for filtering
    full_name = entry.get("full_name", "")
    thm_ns = full_name.rsplit(".", 1)[0] if "." in full_name else ""

    cache_key = (full_path, thm_ns)
    if cache_key in _source_vars_cache:
        return _source_vars_cache[cache_key]
    ...
    try:
        ...
        _source_vars_cache[cache_key] = result
        return result
    ...
    while i < len(lines):
        ...
        # Skip comment lines and indented lines
        if stripped.startswith("--") or (line and line[0:1] in (" ", "\t")):
            i += 1
            continue

        # Track namespace/section stack
        # Both `namespace X` and `section X` create a lexical scope that limits
        # variable visibility. We track both to avoid including out-of-scope vars.
        ns_m = re.match(r'^namespace\s+(\S+)', stripped)
        if ns_m:
            ns_stack.append(ns_m.group(1))
            i += 1
            continue
        sec_m = re.match(r'^section(\s+(\S+))?', stripped)
        if sec_m and stripped.split()[0] == 'section':
            # Named section: push name; unnamed: push '' as marker
            sec_name = sec_m.group(2) or ''
            ns_stack.append(sec_name)
            i += 1
            continue
        end_m = re.match(r'^end\s+(\S+)', stripped)
        if end_m:
            ...
            if ns_stack and ns_stack[-1] == end_name:
                ...
                # Mismatched end — pop anyway to stay consistent
                ns_stack.pop()
            i += 1
            continue
        # Handle bare `end` (closes unnamed section or namespace)
        if stripped == 'end':
            if ns_stack:
                ns_stack.pop()
            i += 1
        ...
        if stripped.startswith("variable"):
            # Check if this variable is in scope for our theorem's namespace.
            # In scope if: (a) at top level (ns_stack empty), OR
            #              (b) current namespace is a prefix of thm_ns
            current_ns = ".".join(ns_stack) if ns_stack else ""
            in_scope = (
                not ns_stack  # top-level variable
                or (thm_ns and (thm_ns == current_ns or thm_ns.startswith(current_ns + ".")))
            )
...
def _normalize_and_dedup_vars(var_decls: list[str]) -> list[str]:
    ...
    Post-process raw variable declarations from the source file:
      ...
      3. Partial-conflict salvage: if a declaration has BOTH named type variables that
         conflict AND unnamed typeclass instances, keep only the typeclass instances
         (as a new `variable [TC1] [TC2]` declaration) rather than dropping everything.
    """
    ...
    for decl in var_decls:
        ...
        # --- Step 2: Conflict detection with partial salvage ---
        body = re.sub(r'^variable\s*', '', decl).strip()
        introduced: set[str] = set()
        conflicting_names: set[str] = set()
        # Collect anonymous typeclass instances [TC X] (no ':' in inner) for salvage
        ...
        for m in _BINDER_RE.finditer(body):
            ...
            if open_b == '[' and ':' not in inner:
                # Anonymous typeclass instance — collect for potential salvage
                anon_instances.append(f"[{inner}]")
                continue
            # Annotation-only binder: {A} or (A) without ':' — binder mode change
            if ':' not in inner:
                names = [n for n in inner.split()
        ...
        if not conflicting_names:
            # No conflicts — keep the whole declaration
            seen_names |= introduced
            result.append(decl)
        elif anon_instances:
            # Partial conflict: keep only the anonymous typeclass instances
            # (they reference existing variables and add useful constraints)
            salvaged = "variable " + " ".join(anon_instances)
            result.append(salvaged)
            # Don't add conflicting names to seen_names (they were already there)
        # else: full conflict with no salvageable parts — drop entirely
...
def _extract_variables(entry: dict) -> list[str]:
    ...
    explicit_params = set(re.findall(r"[({[]\s*([^\W\d]\w*'*)\s*:", statement))
    ...
    for line in state_before.split("\n"):
        ...
        if re.search(r"\bType\b|\bSort\b", type_str) and "→" not in type_str:
            # Handle: Type u, Type u_1, Type uE', Type (max u v)
            normalized = re.sub(r"\bType\s+[\w.']+", "Type*", type_str)
            normalized = re.sub(r"\bSort\s+[\w.']+", "Sort*", normalized)
            normalized = re.sub(r"\bType\s*\([^)]{1,40}\)", "Type*", normalized)
            normalized = re.sub(r"\bSort\s*\([^)]{1,40}\)", "Sort*", normalized)
            for name in names:
        ...
        for name in names:
            if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                variables.append(f"variable ({name} : {type_str})")
...
def build_check_command(entry: dict) -> str:
    ...
    # Prefer source-file variable declarations (ground-truth) over state_before extraction.
    # Source vars use the correct Lean syntax (no dot-notation in types) and proper binders.
    # Fall back to state_before extraction when source vars are unavailable.
    source_vars_raw = _get_source_variables(entry)
    if source_vars_raw:
        var_decls = _normalize_and_dedup_vars(source_vars_raw)
    else:
    ...
    lines += opens
    if ns:
        # Place variable declarations and local notations INSIDE the namespace block.
        # This is essential when variables reference types that are only accessible
        # within the namespace (e.g., `OpenCover X` in `namespace AlgebraicGeometry.Scheme.Pullback`).
