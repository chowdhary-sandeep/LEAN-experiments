...
def _extract_sq_bracket_groups(text: str) -> list[str]:
    """
    Extract balanced [...] groups from text, correctly handling nested parentheses.
    e.g. '[NeZero (2 : K)]' is returned as a single group even though '(2 : K)' is nested.
    The simple _BINDER_RE regex breaks on such nested brackets.
    """
    groups: list[str] = []
    i = 0
    while i < len(text):
...
def _normalize_and_dedup_vars(var_decls: list[str]) -> list[str]:
    ...
    for decl in var_decls:
        ...
        if not has_conflict:
            ...
            # The declaration has a naming conflict, but it may also contain NEW
            # anonymous typeclass instances (e.g. '[NeZero (2 : K)]') that aren't
            # covered by the already-kept declarations.  Extract and keep only the
            # anonymous-instance binders from the conflicting decl so that critical
            # typeclasses like NeZero / CharZero are not silently dropped.
            # Use balanced bracket extraction to correctly handle [TC (nested : args)].
            extra_instances: list[str] = []
            for group in _extract_sq_bracket_groups(body):
                inner = group[1:-1].strip()  # strip [ and ]
                if ':' in inner:
                    # Named instance — skip if the name conflicts
                    names_part = inner.split(':', 1)[0].strip()
                    inst_names = [n for n in names_part.split()
                                  if re.match(r'^[^\W\d]\w*\'*$', n, re.UNICODE)]
                    if any(n in seen_names for n in inst_names):
                        continue
                # Anonymous typeclass binder [TC args] — safe to repeat even if
                # the type arguments overlap, as Lean ignores redundant instances.
                extra_instances.append(f"variable [{inner}]")
            result.extend(extra_instances)
...
def _extract_variables(entry: dict) -> list[str]:
    ...
    # Improved extraction: handles all binder delimiters (including ⦃⦄) and
    # multi-name binders like '(i j : T)' — extracts ALL names before ':'.
    # Filters out Lean keywords that appear in type expressions like 'if p then a else 0'.
    _LEAN_KWS = frozenset({
        "if", "then", "else", "fun", "let", "in", "match", "with",
        "do", "return", "for", "while", "by", "have", "show",
        "from", "calc", "suffices", "obtain", "rcases",
    })
    explicit_params: set[str] = set()
    for _content in re.findall(r'[({[⦃]([^)}\]⦄]+)[)}\]⦄]', statement):
        if ':' not in _content:
            continue
        _names_part = _content.split(':', 1)[0].strip()
        # Only accept binder groups whose names-part is purely identifiers + whitespace.
        # If there are operators / non-identifier chars it's a term expression, not a name list.
        if re.sub(r'[^\W\d]\w*\'*', '', _names_part).strip():
            continue
        for _n in re.findall(r'[^\W\d]\w*\'*', _names_part):
            if _n not in _LEAN_KWS:
                explicit_params.add(_n)

    variables: list[str] = []
    # Track the first type variable with a Ring/Field/Semiring typeclass instance.
    # Used to annotate bare numeric literals in typeclass arguments, e.g.
    # 'inst✝ : NeZero 2' becomes 'variable [NeZero (2 : K)]' when K : Field K is in scope.
    _type_vars_seen: list[str] = []
    _ring_type_var: str = ""
    # Ring-like typeclass prefixes that bind numeric literals to their type argument
    _RING_TC_PREFIXES = (
        "Field", "CommRing", "Ring", "Semiring", "CommSemiring",
        "LinearOrderedField", "LinearOrderedRing", "GroupWithZero",
        "DivisionRing", "EuclideanDomain",
    )
    ...
    for line in state_before.split("\n"):
        ...
        if any(n.lower().startswith("inst") for n in names):
            ...
            # Strip explicit type annotations from ∀-bound variables in typeclass instances.
            # Lean's state_before prints e.g. '∀ (i : 𝒰.J), HasPullback ...' but this
            # dot-notation type causes 'expected token' in variable declarations.
            # The source file just writes '∀ i,' (type inferred), which is more robust.
            tc_type = re.sub(r'∀\s*\(([^\W\d]\w*\'*)\s*:[^)]+\)', r'∀ \1', tc_type)
            # Track which type variable is the "main" ring type for numeric annotation.
            if not _ring_type_var and tc_type.startswith(_RING_TC_PREFIXES):
                for word in tc_type.split():
                    if word in _type_vars_seen:
                        _ring_type_var = word
                        break
            # Annotate bare numeric literals in typeclass args with the ring type.
            # e.g. 'NeZero 2' → 'NeZero (2 : K)' when K is the main ring type var.
            # This prevents 'failed to synthesize NeZero 2' errors in Field contexts.
            if _ring_type_var and re.search(r'\b\d+\b', tc_type):
                tc_type = re.sub(
                    r'\b(\d+)\b',
                    lambda m: f"({m.group(1)} : {_ring_type_var})",
                    tc_type,
                )
        ...
        if all("✝" in n for n in names):
            continue
        # Skip regular variables whose TYPE references inaccessible names (✝).
        # e.g. 'b : ℍ[R,c₁✝,c₂✝]' — the ✝-suffixed names are not in scope.
        if "✝" in type_str:
            continue
        ...
        if re.search(r"\bType\b|\bSort\b", type_str) and "→" not in type_str:
            ...
            for name in names:
                if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                    ...
                    _type_vars_seen.append(name)
            continue

        # Regular variables — only add if not already an explicit param.
        # Also normalize any embedded universe levels in the type (e.g. ι → Type u_3 → ι → Type*).
        _type_norm = re.sub(r'\bType\s+[\w.\']+', 'Type*', type_str)
        _type_norm = re.sub(r'\bSort\s+[\w.\']+', 'Sort*', _type_norm)
        _type_norm = re.sub(r'\bType\s*\([^)]{1,40}\)', 'Type*', _type_norm)
        _type_norm = re.sub(r'\bSort\s*\([^)]{1,40}\)', 'Sort*', _type_norm)
        for name in names:
            if re.match(r"^[^\W\d]\w*'*$", name, re.UNICODE) and name not in explicit_params:
                variables.append(f"variable ({name} : {_type_norm})")
